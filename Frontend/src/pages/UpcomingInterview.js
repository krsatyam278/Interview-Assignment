...
function UpcomingInterview() {
  ...
  return (
    <Container>
      <h2 className="head">Upcoming Interviews</h2>
      {items.interviews?.map((currElem) => {
        return (
          <div className="col d-flex justify-content-center">
            <Card className="mt-2 mb-2" style={{ width: "30rem" }}>
              <Card.Body>
                <Card.Text>
                ...
                <Card.Link href={`/edit/${currElem._id}`}>
                  <button type="button" class="btn btn-success">
                    Edit
                  </button>
                </Card.Link>

                <Card.Link href={"/upcoming"}>
                  {" "}
                  <button
                    type="button"
                    onClick={() => deleteHandler(currElem._id)}
